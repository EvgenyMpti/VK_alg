...
std::vector<ImplementationResult> Task6::runImplementations()
{
    ...
    std::vector<int> copy1 = arr1, copy2 = arr1;

    std::vector<ImplementationResult> results;
    results.push_back(runAndMeasure([&]() { mergeArrays(copy1, arr2); }, "merge Arrays"));
    results.push_back(runAndMeasure([&]() { mergeArraysSort(copy2, arr2); }, "merge & sort"));
